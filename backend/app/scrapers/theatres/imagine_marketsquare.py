...
_BASE_URL = "https://omniwebticketing6.com/imaginecinemas/marketsquare/"
_GMOVIEDATA_RE = re.compile(r"var gMovieData = (\{)")

CONFIG = TheatreConfig(
    slug="imagine_marketsquare",
...
class ImagineMarketSquareScraper(BaseScraper):
            ...
            try:
                ...
                for date in dates:
                    ...
                    try:
                        ...
                        logger.exception("imagine_marketsquare_fetch_error", extra={"date": date_str})
            finally:
                await browser.close()
    ...
    def parse(self, html: str) -> list[RawScreening]:
        ...
        try:
            ...
            logger.exception("imagine_marketsquare_json_error")
            return []
